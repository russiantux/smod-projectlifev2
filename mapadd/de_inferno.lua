...
function SpawnGordon(org, ang)
	...
	if plr ~= nil then
		...
		item = HL2.CreateEntity("weapon_frag", org, ang)
		HL2.SpawnEntity(item)
		item = HL2.CreateEntity("item_rpg_round", org, ang)
		HL2.SpawnEntity(item)
		item = HL2.CreateEntity("weapon_crowbar", org, ang)
		HL2.SpawnEntity(item)
		item = HL2.CreateEntity("weapon_rpg", org, ang)
		HL2.SpawnEntity(item)
	end	
end
...
--these are the combine tools. The rollermine is a real pain without the gravity gun, (which you won't get in this game) but you can avoid it or blow it up with explosives (a direct hit with a grenade will do the job). The turret you will have to knock over with a grenade or run up and grab it and point it at a wall.
...
--this is the rpg ammo you will need to take down the helicopter. 
	function SpawnAmmo2(org, VECTORZERO)
	am = HL2.CreateEntity("item_item_crate", org, VECTORZERO)
		if am ~= nil then
			HL2.KeyValue(am, "ItemClass", "item_rpg_round")
			HL2.KeyValue(am, "ItemCount", HL2.RandomInt(4,5))
			HL2.SpawnEntity(am)
		end
end
...
function CheckEnemy()
	...
	local ent2 = HL2.FindEntityByName(nil, "heli")
	if ent2 ~= nil then found = found + 1
	end
			
	if found == 0 and END_FLG == false and RFC_FLG == true then 

		HL2.CallMapaddLabel("clear")
		END_FLG = true
		
	elseif found < 5 and RFC_FLG == false and END_FLG == false then 
			reinforcements()
			RFC_FLG = true
...
function SpawnExtras(ent)
	...
	local sn = 1
	local sncnt = 1
	local sqn = ZOMBIENUM / SQUADS
	while ent ~= nil do
		
		local ang = HL2.GetEntityAbsAngles(ent)
		local org = HL2.GetEntityAbsOrigin(ent)
		
		local squadname = "zsquad" .. sn
		SpawnZombie(org, ang, squadname)
		sncnt = sncnt + 1
		if sncnt > sqn then 
			sncnt = 1
			sn = sn + 1
...
local it = HL2.CreateEntity("instant_trig", VECTORZERO, VECTORZERO)
	if( it ~= nil ) then
		HL2.KeyValue(it, "label", "heli")	
		HL2.KeyValue(it, "timer", HL2.RandomInt(1,10))
		HL2.SpawnEntity(it)
	end
end
