...
local	AMMONUM = HL2.RandomInt(5,8)
...
local REINFORCEMENTPOINT = "assault_rallypoint"
...
function CheckEnemy()
	...
	elseif found < 3 then 
		local ent2 = HL2.FindEntityByName(nil, "reinforcement_marker")
		if ent2 ~= nil then reinforcements()
...
HL2.ShowHUDMessage("Combine reinforcements inbound!")

local it = HL2.CreateEntity("instant_trig", VECTORZERO, VECTORZERO)
	if( it ~= nil ) then
		HL2.KeyValue(it, "label", "dropship")	
		HL2.KeyValue(it, "timer", HL2.RandomInt(1,5))
		HL2.SpawnEntity(it)
	end
...
function Spawn_Reinforcements()
	...
	cnt = 10
	ent = HL2.FindEntityByClass(nil, REINFORCEMENTPOINT)
	local sn = 1
	local sncnt = 1
	local sqn = COMBINENUM / SQUADS
	while ent ~= nil do
		...
		ent = HL2.FindEntityByClass(ent, REINFORCEMENTPOINT)
