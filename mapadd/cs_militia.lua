...
local	TOOLSNUM = HL2.RandomInt(1,2)
...
local	COMBINESPAWNPOINT = "assault_rallypoint"
local	HOSTAGESPAWNPOINT = "assault_assaultpoint"
...
--these are little surprises that come in when you get to the hostages
function trap1(ent)
	local it = HL2.CreateEntity("instant_trig", VECTORZERO, VECTORZERO)
	if( it ~= nil ) then
		HL2.KeyValue(it, "label", "cann1")	
		HL2.KeyValue(it, "timer", HL2.RandomInt(10,20))
		HL2.SpawnEntity(it)
	end
end
...
function gunship(ent)
	local it = HL2.CreateEntity("instant_trig", VECTORZERO, VECTORZERO)
	if( it ~= nil ) then
		HL2.KeyValue(it, "label", "gunship")	
		HL2.KeyValue(it, "timer", HL2.RandomInt(1,20))
		HL2.SpawnEntity(it)
	end
end
